...
def predictor(ticker):
    ...
    df = yf.download(ticker, start='2015-01-01', end=currentDate)
    ...
    quote = df = yf.download(ticker, start='2021-01-01', end=currentDate)
    print(quote)

    pred_df = quote.filter(['Close'])
    pred_set = pred_df[-60:].values

    for i in range(15):
        pred_set_scaled = scaler.transform(pred_set)

        X_test = []
        X_test.append(pred_set_scaled)
        X_test=np.array(X_test)
        X_test=np.reshape(X_test, (X_test.shape[0], X_test.shape[1], 1))
        ...
        pred_set_temp = np.append(pred_set, [pred_price[0][0]])
        pred_set_temp = np.delete(pred_set_temp, 0, axis = 0)

        pred_set = np.reshape(pred_set_temp, (-1, 1))
    #print("Next 15 days prediction: ")
    print(pred_set[-15:])
...
def news_analysis(ticker):
    ...
    #Adjusting url according to the given ticker
    url = finviz_url + ticker 
   
   #Retrieves latest articles from the news table through the html code of the page
    ...
    parsed_news = []
    #Calculates date range for news articles from current date to past two days
    today = datetime.now()
    subtract_days = timedelta(2)
    past_date = today-subtract_days

    #Scrapes data from the news table
    ...
    #Creating the dataframe with columns and data
    ...
    #Calculates average sentiment value of compound
    mean = round(dataframe['compound'].mean(), 2)

    print("\nMean Sentiment of articles (-1 (Negative), 0 (Neutral), 1 (Positive)): ", mean)

#Calls predictor and news_analysis
